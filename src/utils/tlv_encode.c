...
{
    if (tag == 0 || tag > 0xFFFFFF00) return 0;   /* Invalid tag */
...
int tlv_decode_tag(const uint8_t *in, uint8_t in_len, uint32_t *tag)
{
    ...
    if ((in[0] & 0x1F) != 0x1F) {
        ...
        /* Form 2: multiple bytes — count continuation bytes */
        tag_bytes = 1;
        while (tag_bytes < in_len && (in[tag_bytes] & 0x80)) {
            tag_bytes++;
        }
        if (tag_bytes > 4) return -1;
    }
...
uint8_t tlv_encode_length(uint16_t len, uint8_t *out, uint8_t out_max)
{
    ...
    uint8_t total = 1 + len_bytes;  /* 1 byte for length of lengths + len_bytes */
...
{
    if (!in || !len_out || in_len == 0) return -1;
...
static int dump_entry_raw(const tlv_entry_t *entry, uint8_t *out, uint8_t out_len)
{
    uint8_t tag_bytes = 0;
    uint8_t len_bytes = 0;
    tlv_encode_tag(entry->tag, NULL, &tag_bytes);
    tlv_encode_length(entry->len, NULL + 0, &len_bytes);
    /* Inline computation since we can't pass temp pointers easily: */
    uint8_t buf[6];
    uint8_t tl = 0;
    tlv_encode_tag(entry->tag, buf, &tl);
...
int tlv_dump_raw(const tx_warehouse_t *wh, uint8_t *out, uint8_t out_len)
{
    ...
    for (uint8_t i = 0; i < wh->count; i++) {
        ...
        if (!e->value) continue;  /* Deleted slot */
...
{
    if (!hex_str || !out || !out_len) return -1;
